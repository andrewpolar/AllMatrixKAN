...
//    parameter shortint N_OUTER_LEARNING_SHIFT = 2,
//    parameter shortint MULT_OUTER = 1,
//    parameter shortint DENOMOUTER = 4,
//
//    //training
...
//        range = (N_TARGETMAX - N_TARGETMIN) >> 2;
//    mean = (N_TARGETMAX + N_TARGETMIN) >> 1;
//    left = mean - range;
//    right = mean + range;
//
...
//                    endfunction
//
//                    // Generate 9 bytes (same behavior as calling C 9 times)
...
//        endfunction
//
//        function automatic int ComputeInner(input int diff, input  int delta_shift,
//            input logic signed[31:0] row[0:N_INNER_POINTS - 1], output int offset, output int index);
//    longint Q;
//    index = diff >> > delta_shift;
//    offset = diff & ((1 << delta_shift) - 1);
//    Q = row[index + 1] - row[index];
//    Q *= offset;
//    Q >>>= delta_shift;
//    Q += row[index];
//    return int'(Q);
//        endfunction
//
//        function automatic int ComputeOuter(input int diff, input  int delta_shift,
//            input logic signed[31:0] row[0:N_OUTER_POINTS - 1], output int offset, output int index);
//    longint Q;
//    index = diff >> > delta_shift;
//    offset = diff & ((1 << delta_shift) - 1);
//    Q = row[index + 1] - row[index];
